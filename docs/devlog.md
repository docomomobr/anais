...
**Pausa (madrugada 14/07):** carga de backlinks interrompida em **1.202/2.841** (399 da véspera + 803 desta rodada; **zero erros em toda a operação**; último: `sdbr14-023`). Feitos: idc06, sdbr01–sdbr13 e parte do sdbr14. Retomar com `python3 -u scripts/add_zenodo_backlinks.py --all` (idempotente). Após concluir: (a) nova versão Zenodo de sdnne07-015/032 com os PDFs verdadeiros; (b) cadastro OpenAIRE pendente (**BASE e CORE submetidos em 13/07** — confirmação do usuário em 14/07; CORE avisa por email quando indexar).
