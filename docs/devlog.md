...
**Reversão (mesma noite): OAI-PMH construído.** Argumento do usuário derrubou o "adiado": **61% do acervo (1.890 artigos) tem abstract em inglês** (1.735 keywords EN, 595 títulos EN) — para o público internacional, BASE/CORE/OpenAIRE são canais primários que o Scholar não cobre (sistemas de descoberta de bibliotecas europeias, infraestrutura OpenAIRE). Com custo já reduzido a ~zero (endereço `*.workers.dev`, sem DNS), o pacote foi implementado (commit `9e1ea948a`): `scripts/gerar_oai.py` (catálogo oai_dc trilíngue, datestamps incrementais por hash), `oai/worker.js` (protocolo completo, 11 testes passando incl. paginação dos 3.104), `oai/README.md` (passo a passo). Falta (ações do usuário): deploy do site (leva o catalog.json junto), conta Cloudflare + colar worker, cadastrar CORE/BASE/OpenAIRE.

**Pausa (fim do expediente 13/07):** carga de backlinks interrompida manualmente em **397/2.841 processados** (381 adicionados + 16 da rodada inicial; **zero erros**; último: `sdbr08-056`). Feitos: idc06 completo (53, com conserto do isPartOf), sdbr01–sdbr07 e parte do sdbr08.
...
**Pausa (manhã 14/07):** carga de backlinks parada em **1.616/2.841** (zero erros; último `sdbr16-213` — nacionais quase completos). Retomar: `python3 -u scripts/add_zenodo_backlinks.py --all`. Frente 301 pronta em `redirects-publicacoes/` aguardando deploy do usuário (CF Pages + CNAME, README com passo a passo). Depois da carga: PDFs verdadeiros sdnne07-015/032 no Zenodo; piloto fulltext na fila.

### 2026-07-14 — Operação backlinks Zenodo CONCLUÍDA: 2.841/2.841

Tally final: **1.223 adicionados na última rodada + 1.617 já presentes + 1 falso-erro** (sdsul03-031: timeout no publish com sucesso no servidor; retentativa achou o draft consumido e reportou 404 — verificado publicado com backlink OK). Amostra de verificação pós-conclusão em 4 âmbitos (sdbr02, sdrj03, sdsp07, sdsul08): todos com backlink.

**Resultado: todos os 2.841 registros Zenodo elegíveis apontam agora para a página HTML do próprio artigo** (isIdenticalTo), além da edição (isPartOf). Os 53 do idc06 tiveram também o isPartOf quebrado consertado. Ficam de fora só os 66 do sdnne06 (aguardando normalização dos slugs). Melhoria futura anotada: tratar 404-no-publish-após-timeout como provável sucesso (verificar via GET) em vez de contar erro.

**Piloto fulltext (sdnne10) refinado com o usuário:** notas com numeração recuperada do PDF e chamadas no corpo em sintaxe pandoc `[^n]` (salto recíproco no Hugo); block quotes distinguidas de notas; hierarquia de títulos por caixa (CAPS=##, normal=###; 69/85 artigos têm 2 níveis genuínos); headings de resumo descartados. Preview em `revisao/fulltext-piloto/`. Régua do usuário registrada: só entra no ar "texto direito". Pendente: outliers EN/ES e o teste de estresse sdnne02 (OCR).

### 2026-07-14 — Frente 301 NO AR: publicacoes.docomomobrasil.com migrado para Cloudflare Pages
...
### 2026-07-15 — Auditoria ORCID concluída: 1.524 → 1.259 (265 mal atribuídos removidos)

Segunda rodada sobre os 402 `area_suspeita`: dossiês com evidência ampliada (afiliação dos anais × instituições do perfil, obras COM fonte, dica automática "instituição bate") julgados por 4 revisores LLM paralelos com critérios holísticos (campos adjacentes legítimos, risco de homônimo, ⚠Zenodo não conta). Resultado: 170 confirmados, 213 removidos (homônimos de medicina, enfermagem, letras, agronomia, marketing...), 19 ambíguos decididos pelo usuário (removidos — vazio > errado). Checagem de segurança mecânica (nenhum removido com instituição batendo) + releitura amostral 15/15 antes de aplicar.

**Balanço da auditoria completa**: dos 1.524 ORCIDs, **265 (17%) pertenciam a outras pessoas** — herança do pipeline de atribuição por busca de nome. Restam 1.259 com confiança alta. Pendências leves: 297 `sem_dados` (perfis vazios, nome ok — risco baixo, autoconferência na campanha resolve), bug do hífen na tokenização do script. Vereditos completos em revisao/campanha-autores/orcid-vereditos-area-suspeita.csv; auditoria acumulada em dedup-identidade-externa.csv.

### 2026-07-15 — Campanha: rascunho do e-mail + worker de claims prontos

**Rascunho do e-mail** (revisao/campanha-autores/rascunho-email.md): tom convidativo; conferência da página; passo a passo do Lattes como capítulo de livro **com o macete do popup** (informar primeiro o ISBN do seminário para liberar o quadro, depois incluir o DOI do artigo); URL canônica dos anais; conferência de ORCID. **Decisão de canal: nada de retorno por e-mail** (evita triagem manual/troca de conta) — correções via **formulário com link mágico** e **opt-out de 1 clique**. Texto fecha junto com o worker.

**Worker de claims** (claims/): links HMAC stateless (campanha: token no e-mail dispensa verificação; orgânico: botão "Este sou eu" → e-mail com link via Brevo, rate limit 3/dia), formulário com validação de ORCID em formato + identidade (nome do perfil na API pública — recusa ORCID de terceiros mostrando o dono), fila de claims em D1 **sem escrita no anais.db** (curadoria manual), opt-outs. 10 testes locais. Deploy pendente (usuário: worker + D1 + secrets, README com passo a passo). Depois do deploy: gerar_campanha.py (mala direta com tokens) e botão nas páginas de autor.

**Propagação ao Zenodo concluída (mesma tarde):** `sincronizar_creators_zenodo.py --auditados` corrigiu os creators de **379 registros** (23 já corretos; 402/402, zero erros) — ORCIDs de terceiros removidos e nomes pós-fusão aplicados também no Zenodo, estancando o empurrão do DataCite aos perfis errados. Limite honesto: entradas já empurradas aos perfis alheios não são retratáveis por nós (a pessoa rejeita no próprio perfil — coberto pelo "confira seu ORCID" da campanha).

**Worker de claims NO AR** (mesma tarde): `claims-anais.tesouraria-docomomobr.workers.dev` — usuário criou worker + banco D1 (schema aplicado; console D1 achata linhas: schema precisou de versão sem comentários) + binding DB + secrets (CLAIM_SECRET novo, também no .env do anais p/ a mala direta; BREVO_API_KEY do Pilotis; EMAIL_FROM = remetente verificado do Pilotis). Teste de ponta a ponta em produção: formulário via token real ✓, claim com ORCID validado na API ✓, ORCID de terceiro recusado nomeando o dono ✓, fluxo orgânico entregando e-mail via Brevo ✓. Um claim de teste na fila (obs "TESTE" — descartar na primeira curadoria). Próximos: gerar_campanha.py (mala direta com tokens), botão "Este sou eu" no template, fechamento do texto do e-mail.
