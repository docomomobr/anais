# sdsp09 — Runner de revisão

Pipeline: revisao | Gerado: 2026-03-27 | Artigos: 27
...
- [ ] **0.3b** Extrair fontes plumber
  ```
  python3 scripts/extrair_fontes_plumber.py --slug sdsp09 --profile-only
  python3 scripts/extrair_fontes_plumber.py --slug sdsp09
  ```
...
- [ ] **0.6** Extrair metadados EN: `python3 scripts/extrair_metadados_en.py --slug sdsp09`
- [ ] **0.7** Extrair metadados ES (artigos com locale=es: abstract, keywords do plumber)
- [ ] **0.8** Verificar abstracts + auto-fix
  ```
  python3 scripts/validate_metadata.py --slug sdsp09 --fix
  ```
...
- [ ] **1.1a** Títulos PT: seed + normalizar + revisão LLM
  ...
  python3 scripts/normalizar_maiusculas.py --slug sdsp09 --dry-run
  python3 scripts/normalizar_maiusculas.py --slug sdsp09
  ```
  → Revisão LLM palavra por palavra (ver §1.1a)
- [ ] **1.1b** Títulos EN/ES:
  ```
  python3 scripts/normalizar_titulos_en.py --slug sdsp09 --dry-run
  python3 scripts/normalizar_titulos_en.py --slug sdsp09
  ``` `[SKIP: title_en = 0]`
- [ ] **1.1c** Revisão LLM títulos EN/ES (cada título vs PDF) `[SKIP: title_en = 0]`
- [ ] **1.2a** Refs limpeza base
  ```
  python3 scripts/clean_references.py --slug sdsp09 --dry-run
  python3 scripts/clean_references.py --slug sdsp09
  python3 scripts/check_references.py --slug sdsp09 --summary
  ```
- [ ] **1.2b** Refs sweep (8 passadas)
  ```
  python3 scripts/fix_validation_issues.py --slug sdsp09 --sweep-refs --dry-run
  python3 scripts/fix_validation_issues.py --slug sdsp09 --sweep-refs
  ```
- [ ] **1.2b+** Re-backfills: `python3 scripts/clean_references.py --slug sdsp09`
- [ ] **1.2c** Refs revisão LLM — TODOS os artigos vs fontes (ver §1.2c)
- [ ] **1.3** Keywords
  ```
  python3 scripts/fix_validation_issues.py --slug sdsp09 --clean-keywords --dry-run
  python3 scripts/fix_validation_issues.py --slug sdsp09 --clean-keywords
  ```
- [ ] **1.5** Loop validação: `python3 scripts/fix_validation_issues.py --slug sdsp09 --loop`
...
- [ ] **1.9** ORCID: `python3 scripts/fetch_orcid.py --search --slug sdsp09` → `--review` → `--apply`
...
- [ ] **2.0** Validação final + HTML + commit
  ```
  python3 scripts/validate_metadata.py --slug sdsp09 --fix
  python3 scripts/gerar_revisao_html.py sdsp09
  sqlite3 anais.db .dump > anais.sql
  git add anais.sql revisao/sdsp09-* && git commit -m "sdsp09 revisão automática (Fases 0-2)"
  ```
