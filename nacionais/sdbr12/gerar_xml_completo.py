#!/usr/bin/env python3
"""
Gera Native XML completo com todos os 82 artigos do sdbr12.
...
OUTPUT = Path("/home/danilomacedo/Dropbox/docomomo/26-27/anais/nacionais/sdbr12_fontes/sdbr12_completo.xml")
...
# Excluir os 3 artigos já importados no protótipo
ARTIGOS_JA_IMPORTADOS = {"sdbr12-001", "sdbr12-062", "sdbr12-082"}
...
{submission_file_xml}      <publication xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" locale="pt_BR" version="1" status="3" url_path="" seq="{art_num}" date_published="2017-11-24" section_ref="{secao_ref}" access_status="http://pkp.sfu.ca native.xsd">
...
def main():
    print("Gerando XML completo com 79 artigos restantes (formato OJS 3.3)...")
    print("(Excluindo sdbr12-001, sdbr12-062, sdbr12-082 já importados)")

    # Listar todos os YAMLs exceto os já importados
    yaml_files = sorted(YAML_DIR.glob("sdbr12-*.yaml"))
    artigos = []
    for yf in yaml_files:
        artigo_id = yf.stem
        if artigo_id not in ARTIGOS_JA_IMPORTADOS:
            artigos.append(artigo_id)

    print(f"  Total de artigos a importar: {len(artigos)}")
...
'''
    ...
    for art_num, artigo_id in enumerate(artigos, 4):  # Começa em 4 (depois dos 3 já importados)
...
'''
    ...
    print(f"\n✓ XML completo gerado: {OUTPUT}")
    print(f"  {len(artigos)} artigos, {size_mb:.1f} MB")
