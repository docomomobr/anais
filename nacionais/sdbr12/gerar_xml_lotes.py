#!/usr/bin/env python3
"""
Gera Native XML em LOTES para importação no OJS.
Respeita limite de 20MB por arquivo (PDFs grandes geram lotes individuais).
"""
...
LOTE_SIZE_DEFAULT = 1  # 1 artigo por lote para upload automático
...
def get_pdf_size_mb(artigo_id):
    """Retorna tamanho do PDF em MB."""
...
def main():
    ...
    artigos = [yf.stem for yf in yaml_files]

    # 1 artigo por lote
    lotes = [[a] for a in artigos]

    print(f"Gerando {len(lotes)} lotes...")
    print(f"Total: {len(artigos)} artigos\n")

    for lote_num, artigos_lote in enumerate(lotes):
        output_file = OUTPUT_DIR / f"sdbr12_lote_{lote_num + 1:02d}.xml"

        # Calcular numeração global do primeiro artigo
        art_offset = sum(len(lotes[i]) for i in range(lote_num))

        print(f"Lote {lote_num + 1}: {len(artigos_lote)} artigo(s) - {artigos_lote}")

        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(gerar_cabecalho_xml())

            for i, artigo_id in enumerate(artigos_lote):
                art_num = art_offset + i + 1
                yaml_file = YAML_DIR / f"{artigo_id}.yaml"
        ...
        status = "✓" if size_mb < 20 else "⚠️ GRANDE"
        print(f"  {status} {output_file.name} ({size_mb:.1f} MB)")

    print(f"\n✓ Gerados {len(lotes)} arquivos em {OUTPUT_DIR}")
